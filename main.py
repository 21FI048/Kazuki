...
#from train_github import trainer, evaluation
...
# # ordinalCrossEntoropyLoss
# class OrdinalCrossEntropyLoss(nn.Module):
#     def __init__(self, num_classes, distance_weights=None):
#         """
#         順序クロスエントロピー損失を計算するクラス

#         Parameters:
#         - num_classes (int): クラスの数
#         - distance_weights (list or None): クラス間の距離の重み（オプション）
#         """
#         super(OrdinalCrossEntropyLoss, self).__init__()
#         self.num_classes = num_classes
#         if distance_weights is None:
#             # デフォルトはクラス間の距離が1ずつ増加する
#             self.distance_weights = [1.0] * num_classes
#         else:
#             # 指定された距離重みを使用
...
#         """
#         順序クロスエントロピー損失を計算する関数

#         Parameters:
#         - predicted (Tensor): モデルの予測値（確率分布, shape = [batch_size, num_classes]）
#         - target (Tensor): 正解ラベル（確率分布, shape = [batch_size, num_classes]）

#         Returns:
#         - loss (Tensor): バッチ平均の損失値（スカラー）
#         """
#         batch_size = predicted.size(0)
#         loss = 0.0

#         for i in range(self.num_classes):
#             # 各クラスについて距離に応じたペナルティを計算
#             for j in range(self.num_classes):

                
#                 # 順序に基づく距離（クラス i とクラス j の距離を考慮）
#                 # konngohenkousiteikububun
#                 distance = self.distance_weights[abs(i - j)]  # abs() を使用して整数の絶対値を取得
#                 # Cross-Entropyの計算と距離に基づく重み付け
#                 loss += -target[:, j] * torch.log(predicted[:, j] + 1e-10) * distance

#         # バッチ平均を取る
#         loss = loss.mean()  # バッチ全体の損失をスカラーにまとめる
#         return loss
...
def main(*args):
    ...
    if only_evaluation == 3:
        ...
        if weighted_loss == 'Yes':
            ...
            #criterion_offence_severity = nn.KLDivLoss(weight=dataset_Train.getWeights()[0].cuda())
            #criterion_offence_severity = OrdinalCrossEntropyLoss(num_classes = 4)
            ...
            #criterion_offence_severity = nn.KLDivLoss()
            #criterion_offence_severity = OrdinalCrossEntropyLoss(num_classes = 4)     #offence serverity class nokazu     
...
if __name__ == '__main__':
    ...
    #parser.add_argument("--path_to_model_weights", required=False, type=str, default="", help="14_model.pth.tar")
