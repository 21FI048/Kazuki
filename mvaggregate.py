...
class ViewMaxAggregate(nn.Module):
    ...
    def forward(self, mvimages):
        ...
        # print("mvimages" , mvimages.shape)
        # print("mvimages" , mvimages)
...
#     def forward(self, mvimages):
#         B, V, C, D, H, W = mvimages.shape # Batch, Views, Channel, Depth, Height, Width
#         aux = self.lifting_net(unbatch_tensor(self.model(batch_tensor(mvimages, dim=1, squeeze=True)), B, dim=1, unsqueeze=True))#dim = jigennsuu 
#         #aux1 = self.lifting_net(unbatch_tensor(self.model(batch_tensor(mvimages, dim=1, squeeze=True)), B, dim=1, unsqueeze=True))
#         #aux2 = self.lifting_net(unbatch_tensor(self.model(batch_tensor(mvimages, dim=1, squeeze=True)), B, dim=1, unsqueeze=True))
...
# Class SRVA(nn.Module):
# Class MRVA(nn.Module):
# Class LRVA(nn.Module):
